...
const PROVIDER_PRICE_UNAVAILABLE_MESSAGE = 'No numbers available for this tier.';
...
const CSV_SERVICE_DATA = loadCsvServiceData({ servicesDirectory: path.join(__dirname, 'services') });
 ...
 */
const TIER_PRICING_PKR = CSV_SERVICE_DATA.pricing || {};
const PROVIDER_RANK_CONFIG = CSV_SERVICE_DATA.providerRanks || {};
...
async function buyNumberWithAutoTier(countryId, serviceCode, serviceType, defaultWebsitePricePkr = 0) {
    ...
    for (const provider of providers) {
        ...
        if (!isWebsitePriceAtLeastProviderPrice(exactWebsitePrice, provider)) continue;
