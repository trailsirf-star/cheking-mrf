...
const { loadCsvServiceData } = require('./service-csv-loader');
const {
...
const { createPurchaseFirewall } = require('./purchase-firewall');
let bcrypt = null;
...
const PROVIDER_PRICE_UNAVAILABLE_MESSAGE = STRICT_PRICE_UNAVAILABLE_MESSAGE;
const purchaseFirewall = createPurchaseFirewall({
    axios,
    smsbowerUrl: SMSBOWER_URL,
    apiKey: SMSBOWER_API_KEY,
    exchangeRate: PROVIDER_PKR_EXCHANGE_RATE,
    providerBalanceEpsilon: PROVIDER_BALANCE_EPSILON
});
const PROVIDER_SERVICE_STATUS_CACHE_TTL_MS = Math.max(30000, Number(process.env.PROVIDER_SERVICE_STATUS_CACHE_TTL_MS || 120000));
...
function getProviderPricePkr(provider) {
    return getProviderCostPkrFromProvider(provider, PROVIDER_PKR_EXCHANGE_RATE);
}

function validateProviderPriceBeforePurchase(websitePricePkr, provider) {
    return validateStrictProviderPrice({
        websitePricePkr,
        providerCostPkr: getProviderPricePkr(provider),
        message: PROVIDER_PRICE_UNAVAILABLE_MESSAGE
    });
}

function isWebsitePriceAtLeastProviderPrice(websitePricePkr, provider) {
    return validateProviderPriceBeforePurchase(websitePricePkr, provider).allowed;
}

function getProviderPurchaseCostPkr(result) {
    return getProviderCostPkrFromPurchase(result, PROVIDER_PKR_EXCHANGE_RATE, PROVIDER_BALANCE_EPSILON);
}

function validateProviderPriceAfterPurchase(websitePricePkr, result) {
    return validateStrictProviderPrice({
        websitePricePkr,
        providerCostPkr: getProviderPurchaseCostPkr(result),
        message: PROVIDER_PRICE_UNAVAILABLE_MESSAGE
    });
}

function isWebsitePriceAtLeastProviderPurchaseCost(websitePricePkr, result) {
    return validateProviderPriceAfterPurchase(websitePricePkr, result).allowed;
}

function makeStrictPriceBlockedResponse(details = {}) {
    return {
        success: false,
        error: PROVIDER_PRICE_UNAVAILABLE_MESSAGE,
...
await queryRun(`
    ALTER TABLE unmatched_notifications
    ADD COLUMN IF NOT EXISTS trx_id TEXT
`);

await queryRun(`
    CREATE INDEX IF NOT EXISTS idx_unmatched_notifications_trx_id
    ON unmatched_notifications (trx_id)
`);
...
async function handleIncomingNotificationForPayments(amount, rawText, trxId) {
    ...
    // TRX ID is REQUIRED for automatic Easypaisa verification.
    // Never auto-approve by amount alone.
    if (!normalizedTrxId) {
        await queryRun(
            `
                INSERT INTO unmatched_notifications
                    (amount, trx_id, raw_text)
                VALUES ($1, NULL, $2)
            `,
            [
                normalizedAmount,
    ...
    // First check whether this exact TRX ID was already used.
    const alreadyUsed = await queryOne(
        `
            SELECT id, status
            FROM payment_requests
            WHERE LOWER(TRIM(transaction_id)) = LOWER(TRIM($1))
            LIMIT 1
        `,
        [normalizedTrxId]
    );
    ...
    if (alreadyUsed) {
        return {
            matched: false,
            duplicate: true,
            reason: 'transaction_id_already_submitted',
            existingRequestId: alreadyUsed.id,
            existingStatus: alreadyUsed.status
        };
    }

    // Exact TRX ID + exact amount + Easypaisa + pending.
    const matchRes = await queryOne(
        `
            ...
            WHERE status = 'pending'
              ...
              AND amount = $1
              AND LOWER(TRIM(transaction_id)) = LOWER(TRIM($2))
    ...
    if (matchRes) {
        try {
            ...
            return {
                matched: true,
                requestId: matchRes.id,
                transactionId: normalizedTrxId
            };
        } catch (err) {
            console.error(
                `[payment-auto-verify] auto-approve failed for request ${matchRes.id}:`,
                formatSafeError(err)
            );
    ...
    // Store BOTH amount and TRX ID so a later request can be matched safely.
    await queryRun(
        `
            INSERT INTO unmatched_notifications
                (amount, trx_id, raw_text)
...
async function tryMatchExistingNotificationForRequest(paymentRequestId, amount) {
    ...
    // Get the actual payment request and its submitted TRX ID.
    ...
    // Only Easypaisa can use this automatic notification matcher.
    if (normalizePaymentMethod(paymentRequest.payment_method) !== 'easypaisa') {
        return false;
    }

    if (String(paymentRequest.status || '').toLowerCase() !== 'pending') {
        return false;
    }

    const normalizedTrxId = normalizePaymentReference(
        paymentRequest.transaction_id
    );

    // Never match an Easypaisa notification without a TRX ID.
    if (!normalizedTrxId) {
        return false;
    }
    ...
    const notification = await queryOne(
        `
            ...
            WHERE consumed = FALSE
              AND amount = $1
              AND LOWER(TRIM(trx_id)) = LOWER(TRIM($2))
    ...
    try {
        ...
        await queryRun(
            `
                UPDATE unmatched_notifications
                SET consumed = TRUE
                WHERE id = $1
            `,
            [notification.id]
        ...
        console.error(
            `[payment-auto-verify] auto-approve failed for request ${paymentRequestId}:`,
            formatSafeError(err)
        );
...
const SERVICE_CSV_DIRECTORY = path.join(__dirname, 'services');
let CSV_SERVICE_DATA = loadCsvServiceData({ servicesDirectory: SERVICE_CSV_DIRECTORY, logger: console });
var TIER_PRICING_PKR = CSV_SERVICE_DATA.pricing || {};
var PROVIDER_RANK_CONFIG = CSV_SERVICE_DATA.providerRanks || {};
const SERVICE_COUNTRY_CACHE_STATE = {
    services: Object.freeze({}),
    loadedAt: 0,
    refreshCount: 0
};
const PROVIDER_COUNTRY_AVAILABILITY_CACHE = new Map();

function cloneServiceCountry(country) {
    return { ...country };
}

function freezeServiceCountryList(countries) {
    return Object.freeze((Array.isArray(countries) ? countries : []).map((country) => Object.freeze(cloneServiceCountry(country))));
}

function logServiceCountryCounts(reason, services) {
    const counts = Object.fromEntries(Object.entries(services || {}).map(([serviceType, countries]) => [serviceType, Array.isArray(countries) ? countries.length : 0]));
    console.info('[service-country-cache] country count per service', { reason, counts });
}

function refreshServiceCountryCacheFromCatalog(catalog, reason = 'startup') {
    const nextServices = {};
    Object.values(catalog || {}).forEach((serviceConfig) => {
        const serviceType = String(serviceConfig?.serviceType || '').trim().toLowerCase();
        if (!serviceType || !Array.isArray(serviceConfig?.countries) || !serviceConfig.countries.length) return;
        nextServices[serviceType] = freezeServiceCountryList(serviceConfig.countries);
    });
    SERVICE_COUNTRY_CACHE_STATE.services = Object.freeze(nextServices);
    SERVICE_COUNTRY_CACHE_STATE.loadedAt = Date.now();
    SERVICE_COUNTRY_CACHE_STATE.refreshCount += 1;
    console.info('[service-country-cache] cache refresh', { reason, serviceCount: Object.keys(nextServices).length, refreshCount: SERVICE_COUNTRY_CACHE_STATE.refreshCount });
    logServiceCountryCounts(reason, nextServices);
}

function getCsvServiceCountries(serviceType) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const countries = CSV_SERVICE_DATA.services?.[serviceKey]?.countries;
    return Array.isArray(countries) ? countries.map((country) => ({ ...country })) : [];
}

function getCachedServiceCountries(serviceType, fallbackCountries = []) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const cachedCountries = SERVICE_COUNTRY_CACHE_STATE.services?.[serviceKey];
    const fallbackCount = Array.isArray(fallbackCountries) ? fallbackCountries.length : 0;
    if (Array.isArray(cachedCountries) && cachedCountries.length) {
        if (fallbackCount && fallbackCount < cachedCountries.length) {
            console.warn('[service-country-cache] country removal reason', { serviceType: serviceKey, reason: 'catalog_country_count_below_csv_cache', cachedCount: cachedCountries.length, catalogCount: fallbackCount });
        }
        return cachedCountries.map(cloneServiceCountry);
    }
    console.warn('[service-country-cache] cache expiration', { serviceType: serviceKey, reason: 'missing_or_empty_country_cache', fallbackCount });
    const csvCountries = getCsvServiceCountries(serviceKey);
    if (csvCountries.length) {
        const restoredCountries = freezeServiceCountryList(csvCountries);
        SERVICE_COUNTRY_CACHE_STATE.services = Object.freeze({
            ...(SERVICE_COUNTRY_CACHE_STATE.services || {}),
            [serviceKey]: restoredCountries
        });
        console.info('[service-country-cache] cache refresh', { reason: 'restored_from_csv_data', serviceType: serviceKey, countryCount: restoredCountries.length });
        return restoredCountries.map(cloneServiceCountry);
    }
    if (fallbackCount) {
        const restoredCountries = freezeServiceCountryList(fallbackCountries);
        SERVICE_COUNTRY_CACHE_STATE.services = Object.freeze({
            ...(SERVICE_COUNTRY_CACHE_STATE.services || {}),
            [serviceKey]: restoredCountries
        });
        console.info('[service-country-cache] cache refresh', { reason: 'restored_from_catalog_fallback', serviceType: serviceKey, countryCount: restoredCountries.length });
        return restoredCountries.map(cloneServiceCountry);
    }
    return [];
}

function getServiceCountriesForResponse(serviceConfig) {
    return getCachedServiceCountries(serviceConfig?.serviceType, serviceConfig?.countries);
}

function getProviderAvailabilityCacheKey(serviceType, countryId) {
    return `${String(serviceType || '').trim().toLowerCase()}:${Number(countryId)}`;
}

function rememberProviderCountryAvailability(serviceType, countryId, available, reason, details = {}) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const numericCountryId = Number(countryId);
    if (!serviceKey || !Number.isFinite(numericCountryId)) return;
    const entry = {
        available: Boolean(available),
        reason: String(reason || '').trim() || (available ? 'available' : 'not_available'),
        checkedAt: Date.now(),
        details
    };
    PROVIDER_COUNTRY_AVAILABILITY_CACHE.set(getProviderAvailabilityCacheKey(serviceKey, numericCountryId), entry);
    console.info('[service-country-cache] cache refresh', { reason: 'provider_availability_status', serviceType: serviceKey, countryId: numericCountryId, available: entry.available, statusReason: entry.reason, ...details });
}

function getCachedProviderCountryAvailability(serviceType, countryId) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const numericCountryId = Number(countryId);
    if (!serviceKey || !Number.isFinite(numericCountryId)) return null;
    const key = getProviderAvailabilityCacheKey(serviceKey, numericCountryId);
    const entry = PROVIDER_COUNTRY_AVAILABILITY_CACHE.get(key);
    if (!entry) return null;
    if (Date.now() - Number(entry.checkedAt || 0) > PROVIDER_SERVICE_STATUS_CACHE_TTL_MS) {
        PROVIDER_COUNTRY_AVAILABILITY_CACHE.delete(key);
        console.info('[service-country-cache] cache expiration', { reason: 'provider_availability_status_expired', serviceType: serviceKey, countryId: numericCountryId });
        return null;
    }
    return entry;
}

function mergeProviderStatusIntoCountries(serviceType, countries) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    return (Array.isArray(countries) ? countries : []).map((country) => {
        const priceMap = getTierPriceMap(serviceKey, country?.countryId);
        const availability = getCachedProviderCountryAvailability(serviceKey, country?.countryId);
        if (!priceMap || availability?.available === false) {
            return {
                ...country,
...
function hasCsvService(serviceType) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    return Boolean(CSV_SERVICE_DATA.services?.[serviceKey]);
}
...
function parseExtraServicesFromCatalogText(rawText, catalog) {
    ...
    for (let index = 0; index < tokens.length - 1; index += 1) {
        const nextEntryIsRepeatedLabelCodePattern = tokens[index + 1] === tokens[index + 2]
            && tokens[index + 3]
            && !/\s/.test(tokens[index + 3])
            && tokens[index + 3].length <= 12;
        if (nextEntryIsRepeatedLabelCodePattern) continue;
        const hasRepeatedLabelCodePattern = tokens[index] === tokens[index + 1]
            && tokens[index + 2]
            && !/\s/.test(tokens[index + 2])
            && tokens[index + 2].length <= 12;
        const code = hasRepeatedLabelCodePattern ? tokens[index + 2] : tokens[index];
        const label = hasRepeatedLabelCodePattern ? tokens[index] : tokens[index + 1];
        ...
        if (hasRepeatedLabelCodePattern || tokens[index + 2] === label) {
            index += 2;
        }
    }
...
function extendServiceCatalogWithExtraServices(catalog) {
    ...
    Object.keys(CSV_SERVICE_DATA.services || {}).forEach((serviceType) => {
        if (!extraServicesByServiceType.has(serviceType)) {
            extraServicesByServiceType.set(serviceType, {
                code: serviceType,
                label: serviceType,
                serviceType
    ...
    extraServicesByServiceType.forEach((extraService, serviceType) => {
        const countries = getCsvServiceCountries(serviceType);
        ...
        catalog[serviceType] = {
            ...
            serviceCode: extraService?.code || serviceType,
            countries,
            preserveCountryOrder,
            markTopCountriesGoodOtp: preserveCountryOrder,
            catalogSource: preserveCountryOrder ? 'application-adding' : 'service-csv'
        };
    });
}
...
refreshServiceCountryCacheFromCatalog(serviceCatalog, 'startup');
...
async function fetchProviderTiers(countryId, serviceCode = 'wa') {
    ...
    if (data && typeof data === 'object') {
        ...
        if (!exactCountryNode && !rootServiceNode && Object.keys(data).some((key) => /^\d+$/.test(String(key)))) {
            console.warn('[service-country-cache] country removal reason', { reason: 'provider_partial_country_response_ignored', countryId: Number(countryId), serviceCode, returnedCountryIds: Object.keys(data).filter((key) => /^\d+$/.test(String(key))).slice(0, 10) });
            return [];
        }
...
async function buyNumberFromProviderWithRetry(countryId, provider, serviceCode = 'wa', websitePricePkr = null, maxDurationMs = 30000, delayMs = 3000) {
    let lastError = PROVIDER_PRICE_UNAVAILABLE_MESSAGE;
    const preflight = validateProviderPriceBeforePurchase(websitePricePkr, provider);
    if (!preflight.allowed) return makeStrictPriceBlockedResponse(preflight);
    const deadline = Date.now() + maxDurationMs;
    while (Date.now() < deadline) {
        const remainingMs = deadline - Date.now();
        const result = await buyNumberFromProvider(countryId, provider, serviceCode, websitePricePkr, Math.max(1, Math.min(15000, remainingMs)));
        if (result.success) return result;
        if (result.strictPriceBlocked) return result;
        lastError = result.error || lastError;
        const waitTimeMs = Math.min(delayMs, deadline - Date.now());
        if (waitTimeMs > 0) await waitMs(waitTimeMs);
    }
    return { success: false, error: lastError };
}

async function buyNumberFromProvider(countryId, provider, serviceCode = 'wa', websitePricePkr = null, timeoutMs = 15000) {
    return purchaseFirewall.purchaseNumber({
        countryId,
...
async function buyNumberByTierStrategy(countryId, clientMaxUsd, serviceCode = 'wa', websitePricePkr = null) {
    try {
        const serverWebsitePrice = roundMoney(websitePricePkr);
        if (serverWebsitePrice <= 0) return makeStrictPriceBlockedResponse({ reason: 'missing_server_website_price' });
        const providers = await fetchProviderTiers(countryId, serviceCode);
        const affordableProviders = [];
        for (const provider of providers) {
            const candidatePriceCheck = validateProviderPriceBeforePurchase(serverWebsitePrice, provider);
            if (!candidatePriceCheck.allowed) return makeStrictPriceBlockedResponse(candidatePriceCheck);
            affordableProviders.push(provider);
            if (affordableProviders.length >= 5) break;
        }
        if (!affordableProviders.length) {
            return makeStrictPriceBlockedResponse({ strategy: 'provider_unavailable', reason: 'no_provider_at_website_price' });
        }
        for (const provider of affordableProviders) {
            const startedAt = Date.now();
            let lastError = 'No number from provider';
            while (Date.now() - startedAt < 15000) {
                const result = await buyNumberFromProvider(countryId, provider, serviceCode, serverWebsitePrice);
                if (result.strictPriceBlocked) return result;
                if (result.success) {
                    return {
                        success: true,
        ...
        return makeStrictPriceBlockedResponse({ strategy: 'provider_exhausted', reason: 'provider_attempts_exhausted' });
    } catch (err) {
        return makeStrictPriceBlockedResponse({ strategy: 'provider_unavailable', reason: 'provider_fetch_failed', providerError: err.message });
    }
}

async function buyNumberWithRetry(countryId, baseUsdPrice, maxAttempts = 3, serviceCode = 'wa', websitePricePkr = null) {
    return buyNumberByTierStrategy(countryId, baseUsdPrice, serviceCode, websitePricePkr);
}

async function getBestAvailableNumber(countryId, clientMaxUsd, serviceCode = 'wa', websitePricePkr = null) {
    return buyNumberByTierStrategy(countryId, clientMaxUsd, serviceCode, websitePricePkr);
}
 ...
 *
 * How it works:
 *  - services/*.csv defines, per service + country, an ordered list
 ...
 *
 * To add/edit tiers for a country, edit that service's CSV in services/.
 */
TIER_PRICING_PKR = CSV_SERVICE_DATA.pricing || {};
PROVIDER_RANK_CONFIG = CSV_SERVICE_DATA.providerRanks || {};

function getProviderRank(providerId, serviceType = '', countryId = '') {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const serviceRanks = CSV_SERVICE_DATA.providerRanksByServiceCountry?.[serviceKey]?.[Number(countryId)];
    return serviceRanks?.[Number(providerId)] || PROVIDER_RANK_CONFIG[Number(providerId)] || 'Bronze';
}
...
async function getLiveTiersForCountry(serviceType, serviceCode, countryId) {
    ...
    if (!priceMap) {
        rememberProviderCountryAvailability(serviceType, countryId, false, 'missing_csv_tier_price_map');
        return [];
    }
    let providers = [];
    try {
        ...
        rememberProviderCountryAvailability(serviceType, countryId, false, 'provider_fetch_failed', { error: err.message });
        return [];
    }
    if (!providers.length) {
        rememberProviderCountryAvailability(serviceType, countryId, false, 'provider_returned_no_tiers', { providerCount: 0 });
        return [];
    }
    const tiers = [];
    let tierNum = 1;
    for (const provider of providers) {
        ...
        tiers.push({
            tierNumber: tierNum++,
            price: websitePrice,
          rank: getProviderRank(pid, serviceType, countryId),
            providerId: pid,
            count: provider.count ?? null
        });
    }
    rememberProviderCountryAvailability(serviceType, countryId, tiers.length > 0, tiers.length > 0 ? 'provider_tiers_available' : 'provider_partial_or_unpriced_response', { providerCount: providers.length, tierCount: tiers.length });
    return tiers;
}
...
async function buyNumberWithAutoTier(countryId, serviceCode, serviceType, defaultWebsitePricePkr = 0) {
    const priceMap = getTierPriceMap(serviceType, countryId);
    const exactWebsitePrice = roundMoney(defaultWebsitePricePkr);
    if (exactWebsitePrice <= 0) {
    ...
    const exactPriceProviderIds = priceMap
        ? new Set(Object.keys(priceMap)
            .filter((pid) => roundMoney(priceMap[pid]) === exactWebsitePrice)
            .map((pid) => Number(pid)))
        : null;
    if (exactPriceProviderIds && exactPriceProviderIds.size === 0) {
        return { success: false, error: PROVIDER_PRICE_UNAVAILABLE_MESSAGE, priceUnavailable: true };
    }
    const purchaseDeadline = Date.now() + 30000;
    for (const provider of providers) {
        const pid = Number(provider.provider_id);
        if (Date.now() >= purchaseDeadline) break;
        if (exactPriceProviderIds && !exactPriceProviderIds.has(pid)) continue;
        const candidatePriceCheck = validateProviderPriceBeforePurchase(exactWebsitePrice, provider);
        if (!candidatePriceCheck.allowed) return makeStrictPriceBlockedResponse(candidatePriceCheck);
        const result = await buyNumberFromProviderWithRetry(
            countryId,
            { provider_id: pid, price: Number(provider.price), maxPrice: pkrToUsd(exactWebsitePrice) },
            serviceCode,
            exactWebsitePrice,
        ...
        if (result.strictPriceBlocked) return result;
        if (result.success) {
            result.customPricePkr = exactWebsitePrice;
            return result;
        }
    }
    return { success: false, error: PROVIDER_PRICE_UNAVAILABLE_MESSAGE, priceUnavailable: true };
...
app.post('/api/order/tier', ensureAuth, async (req, res) => {
    ...
    try {
        ...
        const countryObj = getServiceCountriesForResponse(serviceConfig).find((c) => Number(c.countryId) === Number(countryId));
        ...
        if (Number.isFinite(lockedProviderId) && lockedProviderId > 0 && priceMap[lockedProviderId] != null) {
            ...
            for (const provider of liveProviders) {
                ...
                if (visibleTierNumber === tierIndex) {
                    const candidatePriceCheck = validateProviderPriceBeforePurchase(providerWebsitePrice, provider);
                    if (!candidatePriceCheck.allowed) return res.status(409).send(PROVIDER_PRICE_UNAVAILABLE_MESSAGE);
...
app.get('/api/countries', async (req, res) => {
    try {
        const cachedCountries = getServiceCountriesForResponse(serviceCatalog.whatsapp);
        const countries = await decorateCountriesWithOtpQuality('whatsapp', cachedCountries);
        ...
        res.json(mergeProviderStatusIntoCountries('whatsapp', enriched));
    } catch {
        res.status(500).send('Server error');
    }
});
...
app.get('/api/service-catalog', (_req, res) => {
    try {
        const services = Object.values(serviceCatalog).map((serviceConfig) => {
            const hasCsvData = hasCsvService(serviceConfig.serviceType);
            return {
                serviceType: serviceConfig.serviceType,
                label: getServiceCatalogLabel(serviceConfig),
                isBuiltIn: BUILT_IN_SERVICE_TYPES.has(serviceConfig.serviceType),
                comingSoon: !serviceConfig.serviceCode && !hasCsvData,
                canOrder: Boolean(serviceConfig.serviceCode) || hasCsvData,
                preserveCountryOrder: serviceConfig.preserveCountryOrder === true,
                countryCount: getServiceCountriesForResponse(serviceConfig).length
            };
        });
        res.json(services);
    } catch {
...
app.get('/api/facebook/countries', async (req, res) => {
    try {
        const countries = await decorateCountriesWithOtpQuality('facebook', getServiceCountriesForResponse(serviceCatalog.facebook));
        res.json(mergeProviderStatusIntoCountries('facebook', countries));
    } catch {
        res.status(500).send('Server error');
    }
});
...
app.get('/api/services/:service/countries', async (req, res) => {
    ...
    try {
        const serviceType = serviceConfig.serviceType || req.params.service;
        const countries = await decorateCountriesWithOtpQuality(serviceType, getServiceCountriesForResponse(serviceConfig));
        res.json(mergeProviderStatusIntoCountries(serviceType, countries));
    } catch {
        res.status(500).send('Server error');
    }
});
...
app.post('/api/order', ensureAuth, async (req, res) => {
    ...
    try {
        ...
        const countryObj = getServiceCountriesForResponse(serviceConfig).find((c) => c.name === countryName && Number(c.countryId) === Number(countryId));
        ...
        const fallbackOrderPrice = roundMoney(countryObj.price || 0);
        const requestedDisplayedPrice = roundMoney(req.body.displayedPricePkr || fallbackOrderPrice);
        if (requestedDisplayedPrice !== fallbackOrderPrice) {
            return res.status(409).send(PROVIDER_PRICE_UNAVAILABLE_MESSAGE);
        }
        const latestProviderSnapshot = await getLatestProviderBalanceSnapshot();
        const providerBalanceBeforePurchase = await getProviderBalance();
        ...
        const orderPrice = roundMoney(fallbackOrderPrice);
        if (orderPrice <= 0) {
            return res.status(400).send('Price not configured for this country');
        }
        if (result.customPricePkr != null && roundMoney(result.customPricePkr) !== orderPrice) {
            return res.status(409).send(PROVIDER_PRICE_UNAVAILABLE_MESSAGE);
        }
        // ---- end auto tier ----
