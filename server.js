...
async function decorateCountriesWithOtpQuality(serviceType, countries) {
    ...
    const tierMergedCountries = mergeCountriesWithConfiguredTiers(normalizedServiceType, countries);
    const baseCountries = preserveCountryOrder
        ? tierMergedCountries
        : sortCountriesByPrice(tierMergedCountries);
 ...
 *
 * To add/edit tiers for a country, edit that service folder's tiers.csv.
 */
const TIER_PRICING_PKR = tiers.pricing;
const PROVIDER_RANK_CONFIG = tiers.providerRanks;
const TIER_COUNTRY_NAMES = tiers.countryNames || {};

function getConfiguredTierCountryIds(serviceType) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const serviceTiers = TIER_PRICING_PKR[serviceKey];
    if (!serviceTiers || typeof serviceTiers !== 'object') return [];
    return Object.keys(serviceTiers).map(Number).filter(Number.isFinite);
}

function getCheapestConfiguredTierPrice(serviceType, countryId) {
    const priceMap = getTierPriceMap(serviceType, countryId);
    if (!priceMap) return null;
    const prices = Object.values(priceMap).map(Number).filter(Number.isFinite);
    return prices.length ? Math.min(...prices) : null;
}

function mergeCountriesWithConfiguredTiers(serviceType, countries) {
    const serviceKey = String(serviceType || '').trim().toLowerCase();
    const list = Array.isArray(countries) ? [...countries] : [];
    const seenCountryIds = new Set(list.map((country) => Number(country.countryId)).filter(Number.isFinite));
    getConfiguredTierCountryIds(serviceKey).forEach((countryId) => {
        if (seenCountryIds.has(countryId)) return;
        seenCountryIds.add(countryId);
        const tierPrice = getCheapestConfiguredTierPrice(serviceKey, countryId);
        list.push({
            name: TIER_COUNTRY_NAMES[serviceKey]?.[countryId] || `Country ${countryId}`,
            code: 'N/A',
            price: tierPrice ?? 0,
            countryId,
            flag: ''
...
app.get('/api/service-catalog', (_req, res) => {
    try {
        const services = Object.values(serviceCatalog).map((serviceConfig) => {
            const hasConfiguredTiers = getConfiguredTierCountryIds(serviceConfig.serviceType).length > 0;
            return {
                serviceType: serviceConfig.serviceType,
                label: getServiceCatalogLabel(serviceConfig),
                isBuiltIn: BUILT_IN_SERVICE_TYPES.has(serviceConfig.serviceType),
                comingSoon: !serviceConfig.serviceCode && !hasConfiguredTiers,
                canOrder: Boolean(serviceConfig.serviceCode) || hasConfiguredTiers,
                preserveCountryOrder: serviceConfig.preserveCountryOrder === true,
                countryCount: mergeCountriesWithConfiguredTiers(serviceConfig.serviceType, serviceConfig.countries).length,
                hasConfiguredTiers
            };
        });
        res.json(services);
