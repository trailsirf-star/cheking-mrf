...
function normalizePaymentModalStatus(status) {
    const normalized = String(status || 'pending').trim().toLowerCase();
    if (normalized === 'approved' || normalized === 'accepted' || normalized === 'success') return 'approved';
    if (normalized === 'cancelled' || normalized === 'canceled' || normalized === 'rejected' || normalized === 'failed') return 'cancelled';
    return 'pending';
}

function getLatestPaymentRequestForModal() {
    return Array.isArray(state.paymentRequests) && state.paymentRequests.length
        ? [...state.paymentRequests].sort((left, right) => new Date(right.created_at) - new Date(left.created_at))[0]
        : null;
}

function isLateNightPaymentWindow(date = new Date()) {
    const hour = date.getHours();
    return hour >= 23 || hour < 7;
}

function syncPaymentNightNotice() {
    const notice = qs('payment-night-notice');
    if (!notice) return;
    notice.classList.toggle('hidden', !isLateNightPaymentWindow());
}

function setPaymentTimelineStatus(status) {
    const normalized = normalizePaymentModalStatus(status);
    qsa('[data-payment-timeline-step]').forEach((step) => {
        const key = step.dataset.paymentTimelineStep;
        step.classList.remove('is-active', 'is-complete', 'is-cancelled');
        if (normalized === 'cancelled') {
            if (key === 'submitted' || key === 'review') step.classList.add('is-complete');
            if (key === 'approved') step.classList.add('is-cancelled');
            return;
        }
        ...
        if (key === 'submitted') step.classList.add('is-complete');
        if (key === 'review') step.classList.add('is-active');
    });
}

function getPaymentRequestAdminRemarks(request) {
    return String(request?.admin_remarks || request?.admin_remark || request?.remarks || request?.remark || request?.cancel_reason || request?.rejection_reason || request?.reason || '').trim();
}

function renderPaymentLatestDetailCard(request = null) {
    const container = qs('payment-latest-detail-card');
    if (!container) return;
    if (!request) {
        container.innerHTML = '';
    ...
    const normalized = normalizePaymentModalStatus(request.status || 'pending');
    const methodMeta = getPaymentMethodMeta(request.payment_method);
    const remarks = getPaymentRequestAdminRemarks(request);
    const proofUrl = request.screenshot ? getUploadUrl(request.screenshot) : '';
    const showNewPayment = normalized === 'approved' || normalized === 'cancelled';
    container.innerHTML = `
        <div class="payment-latest-grid">
            <div class="payment-latest-item">
                <div class="payment-latest-label">Amount</div>
                <div class="payment-latest-value">${escapeHtml(formatPaymentRequestAmountText(request))}</div>
            </div>
            <div class="payment-latest-item">
                <div class="payment-latest-label">Payment Method</div>
                <div class="payment-latest-value">${escapeHtml(methodMeta.label)}</div>
            </div>
            <div class="payment-latest-item">
                <div class="payment-latest-label">Submitted Time</div>
                <div class="payment-latest-value">${escapeHtml(formatRelativeTime(request.created_at))}</div>
            </div>
            <div class="payment-latest-item">
                <div class="payment-latest-label">Reference</div>
                <div class="payment-latest-value">${escapeHtml(getPaymentRequestReferenceValue(request))}</div>
            </div>
        </div>
        <div class="payment-admin-remarks ${normalized === 'cancelled' && remarks ? '' : 'hidden'}"><strong>Admin Remarks:</strong> ${escapeHtml(remarks)}</div>
        <div class="payment-status-actions">
            ${proofUrl ? `<button type="button" class="payment-status-action payment-status-action--proof" data-action="view-payment-proof" data-image="${escapeAttr(proofUrl)}" data-user="${escapeAttr(request.user_name || 'Customer')}" data-email="${escapeAttr(request.user_email || 'Unknown email')}" data-amount="${escapeAttr(formatPaymentRequestAmountText(request))}" data-status="${escapeAttr(formatPaymentStatus(request.status || 'pending'))}">View Screenshot</button>` : ''}
            <button type="button" class="payment-status-action payment-status-action--refresh ${normalized === 'pending' ? '' : 'hidden'}" data-action="refresh-payment-status">Refresh Status</button>
            <button type="button" class="payment-status-action payment-status-action--new ${showNewPayment ? '' : 'hidden'}" data-action="make-new-payment">Make New Payment</button>
        </div>
    `;
    container.classList.remove('hidden');
}

function setPaymentModalStatus(status = 'pending', request = null) {
    const normalized = normalizePaymentModalStatus(status);
    ...
    if (hasRequest) {
        qs(`payment-status-message-${normalized}`)?.classList.add('is-visible');
    }
    const summary = qs('payment-status-summary');
    if (summary) {
        if (!hasRequest) {
            summary.textContent = '';
        } else if (normalized === 'approved') {
            summary.textContent = 'Approved. You can make a new payment now.';
        } else if (normalized === 'cancelled') {
            summary.textContent = 'Cancelled. You can make a new payment now.';
        } else {
            summary.textContent = 'Waiting for Admin Approval.';
        }
    }
    const updated = qs('payment-status-updated');
    if (updated) {
        updated.textContent = hasRequest && request?.created_at ? formatRelativeTime(request.created_at) : '';
    }
    renderPaymentLatestDetailCard(request || null);
    setPaymentTimelineStatus(normalized);
}

function syncPaymentPageVisibility(latestRequest = null) {
    const normalized = normalizePaymentModalStatus(latestRequest?.status || 'pending');
    const hasRequest = Boolean(latestRequest);
    const isPending = hasRequest && normalized === 'pending';
    qs('payment-form-view')?.classList.toggle('hidden', isPending);
    qs('payment-success-view')?.classList.add('hidden');
    qs('payment-modal')?.classList.toggle('has-payment-status', hasRequest);
    qs('payment-modal')?.classList.toggle('has-pending-payment', isPending);
    qsa('.payment-status-section').forEach((section) => section.classList.toggle('hidden', !hasRequest));
    qsa('.payment-timeline-section').forEach((section) => section.classList.add('hidden'));
    if (!hasRequest) {
        renderPaymentLatestDetailCard(null);
    }
}

function syncPaymentModalStatusFromHistory(history = state.paymentRequests) {
    const latestRequest = Array.isArray(history) && history.length
        ? [...history].sort((left, right) => new Date(right.created_at) - new Date(left.created_at))[0]
        : getLatestPaymentRequestForModal();
    setPaymentModalStatus(latestRequest?.status || 'pending', latestRequest || null);
    syncPaymentPageVisibility(latestRequest || null);
}

function preparePaymentFormForNewPayment() {
    resetPaymentModalState({ preserveHistoryState: true });
    qsa('.payment-status-section, .payment-timeline-section').forEach((section) => section.classList.add('hidden'));
    renderPaymentLatestDetailCard(null);
    qs('payment-form-view')?.classList.remove('hidden');
    qs('payment-form-view')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

async function refreshPaymentCenterStatus(button = null) {
    ...
    try {
        ...
        syncPaymentModalStatusFromHistory(history);
        showToast('Payment status refreshed', 'success');
    } catch {
        showToast('Payment status could not be refreshed', 'error');
    } finally {
        if (button) {
            button.disabled = false;
...
function setActivePaymentMethod(method = 'easypaisa') {
    ...
    const latestRequest = getLatestPaymentRequestForModal();
    if (normalizePaymentModalStatus(latestRequest?.status || '') === 'pending') {
        syncPaymentPageVisibility(latestRequest);
    }
    syncPaymentNightNotice();
...
function updatePaymentSuccessView(method, result = {}) {
    ...
    if (result?.paymentLabel || result?.id || result?.status) {
        setPaymentModalStatus('pending', result);
    }
}
...
function syncBalanceBannerVisibility() {
    ...
    if (!banner) return;
    const paymentPage = qs('payment-modal');
    banner.style.display = paymentPage && !paymentPage.classList.contains('hidden') ? 'none' : 'flex';
}
...
function resetPaymentModalState(options = {}) {
    const preserveHistoryState = Boolean(options.preserveHistoryState);
    ...
    qs('payment-screenshot-input')?.closest('.payment-upload-box')?.classList.remove('is-selected');
    updatePaymentSuccessView('easypaisa');
    setActivePaymentMethod('easypaisa');
    if (!preserveHistoryState) {
        syncPaymentModalStatusFromHistory();
    }
    syncPaymentNightNotice();
}
...
function closePaymentPage() {
    ...
    qs('dashboard-page')?.classList.remove('hidden');
...
async function openPaymentModal() {
    ...
    qs('payment-page-loading')?.classList.remove('hidden');
    qsa('.payment-status-section, .payment-timeline-section').forEach((section) => section.classList.add('hidden'));
    qs('dashboard-page')?.classList.add('hidden');
    qs('payment-modal')?.classList.remove('hidden');
    document.body.classList.remove('overflow-hidden');
    const history = await loadPaymentHistory();
    qs('payment-page-loading')?.classList.add('hidden');
    syncPaymentModalStatusFromHistory(history);
    syncPaymentNightNotice();
    window.setTimeout(() => qs('payment-modal')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 30);
}
...
async function orderCountry(name, id) {
    ...
    try {
        const response = await fetch('/api/order', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ countryName: name, countryId: Number(id), service: selectedService, displayedPricePkr: Number(selectedCountry?.price || 0), idempotencyKey }),
            credentials: 'include',
        ...
        if (/balance|wallet|insufficient/i.test(message)) {
            showToast('Insufficient Balance', 'info', 3200);
            return;
        }
        showToast(message, 'error');
    } finally {
...
function bindStaticEvents() {
    ...
    qs('close-payment-modal')?.addEventListener('click', closePaymentPage);
    ...
    qs('payment-screenshot-input')?.addEventListener('change', (event) => {
        ...
        const uploadBox = event.target.closest('.payment-upload-box');
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (action === 'refresh-payment-status') {
            await refreshPaymentCenterStatus(actionTarget);
            return;
        }
        if (action === 'make-new-payment') {
            preparePaymentFormForNewPayment();
            return;
        }
        if (action === 'copy-account-number') {
            copyTextWithFeedback(actionTarget, '03439898333', 'Copied ✓');
            return;
        }
        if (action === 'copy-payment-target') {
            copyTextWithFeedback(actionTarget, actionTarget.dataset.copyValue || '', 'Copied ✓');
            return;
        }
