...
function applyFallbackIfEmpty(serviceCountries, allowFallback = true) {
    if (allowFallback && (!serviceCountries || serviceCountries.length === 0)) {
...
function normalizeServiceCatalogEntry(entry) {
    ...
    return {
        ...
        countryCount: Number.isFinite(Number(entry?.countryCount)) ? Number(entry.countryCount) : null,
        hasConfiguredTiers: entry?.hasConfiguredTiers === true
    };
}
...
function mergeServiceCatalogEntries(primaryServices, fallbackServices) {
    const merged = [];
    const seen = new Set();
    const primaryByType = new Map((Array.isArray(primaryServices) ? primaryServices : [])
        .filter((service) => service?.serviceType)
        .map((service) => [service.serviceType, service]));
    const addService = (service) => {
        if (!service || !service.serviceType || seen.has(service.serviceType)) return;
        seen.add(service.serviceType);
        merged.push(service);
    };
    (Array.isArray(fallbackServices) ? fallbackServices : []).forEach((service) => {
        addService(primaryByType.get(service?.serviceType) || service);
    });
...
async function loadCountries() {
    try {
        ...
        const catalogEntry = getCurrentServiceCatalogEntry();
        const allowFallback = !catalogEntry || catalogEntry.countryCount == null;
        state.allCountries = applyFallbackIfEmpty(Array.isArray(countries) ? countries : [], allowFallback).map((country, index) => ({
            ...country,
            catalogOrder: index
        }));
        renderCountries();
