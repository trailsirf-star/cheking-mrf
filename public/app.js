...
function setPaymentModalStatus(status = 'pending', request = null) {
    ...
    qsa('[data-payment-status-card]').forEach((card) => {
        card.classList.toggle('is-active', card.dataset.paymentStatusCard === normalized);
    });
    ...
    if (summary) {
        if (normalized === 'approved') {
            ...
            summary.textContent = request ? 'Waiting for Admin Approval.' : 'Submit a payment to begin review.';
        }
    }
    const updated = qs('payment-status-updated');
    if (updated) {
        updated.textContent = request?.created_at ? formatRelativeTime(request.created_at) : 'Live';
    }
...
function updatePaymentSuccessView(method, result = {}) {
    ...
    setPaymentModalStatus('pending', result || null);
}
