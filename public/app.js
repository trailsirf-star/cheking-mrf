...
function setPaymentModalStatus(status = 'pending', request = null) {
    ...
    if (summary) {
        if (!hasRequest) {
            ...
            summary.textContent = 'Your latest payment request has been accepted.';
        } else if (normalized === 'cancelled') {
            summary.textContent = 'Your latest payment request was cancelled.';
        } else {
...
function syncPaymentPageVisibility(latestRequest = null) {
    ...
    qsa('.payment-status-section, .payment-timeline-section').forEach((section) => {
        section.classList.toggle('hidden', !hasRequest);
    });
}

function syncPaymentModalStatusFromHistory(history = state.paymentRequests) {
    const latestRequest = Array.isArray(history) && history.length ? history[0] : getLatestPaymentRequestForModal();
...
function setActivePaymentMethod(method = 'easypaisa') {
    ...
    syncPaymentPageVisibility(getLatestPaymentRequestForModal());
